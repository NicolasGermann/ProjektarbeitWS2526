...
namespace HTW.Influx.Extention {
  public static class InfluxExtention {
    public static PrinterDTO ConnectToDatabase(this PrinterDTO pr,
                                               InfluxDBDTO db) {
      ...
      Thread thread =
                           ...
                           while (true) {
                             try {
                               if (pr.Messages.TryDequeue(out var msg)) {
                                 var result =
                                     JasonToInflux.JsonToInfluxPoint(msg, pr);
        ...
        case Result<(PointData Point, string Measurement, List<string> FieldNames, string? JobId, string? GcodeState)>.Success(var built):
          try {
            ...
            writeApi.Flush();
            var jobLabel =
                string.IsNullOrWhiteSpace(built.JobId) ? "no-job" : built.JobId;

            Console.WriteLine(
                $"[INFLUX] job_id={jobLabel} measurement=\"{built.Measurement}\" fields=[{string.Join(", ", built.FieldNames)}]");
          } catch (Exception e) {
            onsole.WriteLine(
      ...
      return pr with { database =
                           db with { dbClient = dbc, runnerThread = thread } };
    }
  }
}