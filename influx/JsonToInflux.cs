...
public static class JasonToInflux {
    public static Result<(PointData Point, string Measurement, List<string> FieldNames, string? JobId, string? GcodeState)> JsonToInfluxPoint(string jsonString, PrinterDTO pr)
    {
        try {
            ...
            PointData pointData = PointData.Measurement(measurementName).Tag("serial", pr.ID).Tag("device", pr.Name);
    ...
    {
        return string.Equals(state, "RUNNING", StringComparison.OrdinalIgnoreCase) || string.Equals(state, "PAUSED", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static string? ConvertJsonValueToString(object value)
    {
        if (value is JsonElement jsonElement) {
            return jsonElement.ValueKind switch { JsonValueKind.String => jsonElement.GetString(), JsonValueKind.Number => jsonElement.ToString(), JsonValueKind.True => "true",
                JsonValueKind.False => "false", JsonValueKind.Null => null, JsonValueKind.Array => jsonElement.ToString(), JsonValueKind.Object => jsonElement.ToString(),
